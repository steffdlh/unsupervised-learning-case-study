...
topics_df["growth_rate"] = topics_df.groupby("topic")["month_year"].transform(
    lambda x: x.value_counts().pct_change().mean() * 100
)

# get the last 6 months
last_6_months = topics_df["update_date"].max() - pd.DateOffset(months=12)
top_topics = (
    topics_df[topics_df["update_date"] >= last_6_months]
    .groupby("topic")["growth_rate"]
    .mean()
    ...
    csv_data.append(
        {
            ...
            "growth_rate": topics_df["growth_rate"].iloc[topic],
            "filename": f"topics_over_time_{topic}.png",
        }
    )
