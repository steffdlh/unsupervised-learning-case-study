...
def load_data():
    ...
    documents = pd.read_pickle("unsupervised_learning_case_study/data/arxiv-metadata-oai-snapshot-cs.pkl")
    documents = documents[pd.to_datetime(documents["update_date"]) > pd.to_datetime("2020-01-01")]
...
def predict_topics(embeddings, abstracts, update_dates):
    ...
    df = pd.DataFrame(
        {"abstract": abstracts, "topic": topics, "update_date": update_dates, "probabilities": _}
    )
    print(df.head(50))
    print("There are {} unique topics".format(len(df["topic"].unique())))
    print(
        f"Count of -1 topic in df: {len(df[df['topic'] == -1])} which is a percentage of {len(df[df['topic'] == -1])/len(df)*100}%"
    )
